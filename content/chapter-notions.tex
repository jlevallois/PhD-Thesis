...
Pour certaines preuves mathématiques, une correspondance précise entre les points $\vx \in \dS$ et $\hat{\vx}\in \partial\Body{\DigShape}{h}$ est nécessaire. Pour un forme $\Shape \subset \R^2$ dont son bord est à courbure bornée $\kappa_{\max}$,la correspondance est nommée la \colorize{projection inverse} ou \anglais{back-projection} (voir la \RefFigure{fig:IIPottmann}). Plus formellement, soit $\NormalDir(\Shape,\vx,l)$ le segment centré au point $\vx$, aligné le long de la normale au point $\vx$ et de demi-longueur $l$. Alors:
%
\begin{definition}{\fakeTitle{Back-projection $\Proj$ \cite{Lachaud2006}}}
\label{def:projection}
%
  Pour $0 < h \le 1/{\kappa_{\max}}$, soit $\Proj: \partial\Body{\DigShape}{h}
  \rightarrow \dS$, $\hat{\vx} \mapsto \vx=\Proj(\hat{\vx})$, où $\vx$ est le
  seul point tel que $\hat{\vx} \in \NormalDir(\Shape,\vx,\frac{\sqrt{2}}{2}h)$.
%
\end{definition}
%
Lemma B.9 \cite{Lachaud:2006-hdr} indicates that the map $\Proj$ is
well-defined and onto. Lemma B.10 further tells that this map is
continuous. It shows that the boundaries $\hBd{h} X$ and $\dS$
are indeed close, since their Hausdorff distance is no greater than
$\frac{\sqrt{2}}{2}h$.

\section{Voisinages}
\label{sec:voisinage}
%
% 4-voisinage relation d'adjacence par arêtes
% 8-voisinage relation d'adjacence par arêtes et sommets
...
\end{definition}
%
Il apparait alors clairement que, par exemple, le volume d'une forme est une quantité géométrique « globale », tandis que la normale, la courbure ou encore la tangente sur sa surface sont des quantités géométriques dites « locales ».
...
Alors, nous pouvons étudier la convergence asymptotique de ces estimateurs (\RefDefinitionFake{2.10}{Klette2004}) :
%
...
\subsection{Classe $C^1$, $C^2$, $C^d$}
%
\begin{definition}{\fakeTitle{Classe $C^1$}}
  %
...
\end{definition}
%
\begin{definition}{\fakeTitle{Classe $C^2$}}
  %
...
\end{definition}
%
\begin{definition}{\fakeTitle{Classe $C^d$}}
  %
