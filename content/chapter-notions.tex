...
En dimension 2 (\respp dimension 3), il s'agit de la région interpixels (\resp
intervoxels) entre les pixels (\resp voxels) à l'intérieur de l'objet digital de
ceux à l'extérieur. Plus concrétement, les surfaces digitales sont des ensembles
...
suivi de contour. En effet, grâce aux relations d'adjacence est à la définition
de la topologie, les algorithmes d'extraction de surface par suivi puisqu'il
s'agit uniquement de résoudre un graphe de surfels.
...
\cad son bord topologique (en orange foncé sur la \RefFigure{fig:frontier}). Nous pouvons également noter $\partial \Body{\DSh}{h}$ le bord topologique du plongement euclidien de $\DSh$.
% La
...
\begin{figure}[ht]
  ...
  bleu}) composé de lignels et surfels. \label{fig:frontier}}
  %
\end{figure}
...
inverse} ou \anglais{back-projection} \cite{Lachaud2006} :
%
...
$\partial\Body{\DSh}{h}$ avec un $h$ adéquat. En dimension supérieure, nous pouvons citer les travaux de \cauthors{Lachaud}{Lachaud2015}
%
...
euclidien de sa discrétisation $\partial\Body{\DSh}{h}$. Un estimateur local
digital estime la quantité géométrique sur tout point $\hat{\vx}$ de
$\partial\Body{\DSh}{h}$.
%
...
l'estimation est donnée sur un élement de la surface combinatoire (une surfel ou
...
d'estimation est en $O(h^a)$, avec la constance $a < 1$ et $h$ qui tend vers
$0$. Cette valeur de $a$ nous permet alors de comparer les vitesses de
convergence des estimateurs. Nous nous intéressons dans cette thèse à la
