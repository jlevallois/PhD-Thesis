...
segments de droite discrète maximaux (\RefSectionsN{sec:segments}{sec:aire-volume-moments}).
...
%
Le concept mathématique des moments (\anglais{General moment theory}) a beaucoup
...
étudiés principalement la reconnaissance de formes (\cauthors{Trier}{Trier1996}
pour une revue sur la reconnaissance de caractères, \cite{Dudani1977}) et la
reconstruction d'objets \cite{Ghorbel2005}. La première utilisation des moments
en analyse d'images remonte à 1962 par \cauthor{Hu}{Hu1962} pour la
...
\begin{figure}[ht]{
    ...
    \end{center}}
    \caption{Plans des courbures principales \cite{WikipediaCurv}.\label{fig:dss-figure}}
\end{figure}
...
la courbure de $f_2$ au point $\vx$. Si $f_1$ et $f_2$ sont deux fois dérivables
en $\vx$, alors la \colorize{courbure moyenne} $\MeanCurv(\vx)$ est définie
comme étant :
%
\begin{equation}
  %
...
uniquement aux pavages discrets par carrés (ou des cubes en dimension
...
pouvons citer les travaux de \cauthors{Lachaud}{Lachaud2015}, qui metttent aussi
...
d'estimation est en $O(h^a)$, avec une constance $a > 0$ et $h$ qui tend vers
