...
\\
%
Ils définissent alors une analyse en espace d'échelle de $|n_R|$ la distance au barycentre ($\vec{xb}$) comme fonction du rayon du voisinage $R$. Ils démontrent
...
classification et ne l'évaluent pas expérimentalement, probablement à cause de la difficulté à distinguer les deux comportements (linéaire et quadratique) comme le montre la \RefFigure{fig:moment-multiscale}.
%
\\
%
...
\begin{figure}[ht]
  ...
  \caption[Résultat de l'\RefEquation{eq:VCM-ratio-eigenvalues}]{Résultat de l'\RefEquation{eq:VCM-ratio-eigenvalues} avec comme paramètres $R = 1$, $T = 50$ pour deux tailles de voisinage $r = 10$ et $r = 22$.}\label{fig:VCM-cubesphere}
\end{figure}
%
...
\\
%
Dans leurs travaux \cite{Mellado2012}, \nauthor{Mellado} proposent de calculer
...
sphères (\emph{least squares spherical fitting}). Le paramètre d'espace d'échelle
...
étudiant les estimateurs de courbures par intégration présentés
précédemment (\RefSection{sec:estimators:volume}). Nous
allons fixer la résolution $h$ d'une forme $\DigShape \subset \Z^d$ et considérer la taille de la sphère d'intégration $R$ comme paramètre de l'espace d'échelle. Ce changement de rayon de sphère d'intégration fera varier le résultat
...
\end{figure}
%
Nous proposons alors d'étudier le comportement des estimateurs digitauxpar intégration lorsque nous réduisons le rayon de la boule
pour une forme et une résolution donnée. La \RefFigure{fig:curvature-scale-2d}
nous montre les variations de courbure en fonction du rayon de la boule
...
fonction du rayon de la boule alors que les points autour de zones lisses sont
relativement insensible au rayon. Pour analyser ce comportement en
espace d'échelle, nous proposons de classifier les points de notre forme en trois
...
\end{definition}
%
Dans la suite, nous assumons que l'objet $\Shape$ a un bord $C^3$ par
...
\paragraph{Cas lisse.}
%
Dans le cas lisse, \cad lorsque $\partial \Shape$ est $C^3$ au point $\vx$, il
...
la valeur retournée n'est plus uniquement liée à la courbure au point $\vx$. Soit
$\Curv_0$ la valeur de courbure au point $\vx$. Puisque nous sommes dans les mêmes
conditions que le théorème~\ref{theo:pottmann-2d-3d-mean-conv}, nous savons que :
%
\begin{equation}
\FeatDD(R) = \CurvT{}(R,\vx) = \Curv_0 + O(R),
\quad \FeatTD(R) = \MeanCurvT{}(R,\vx) = \MeanCurv_0 + O(R),
\end{equation}
%
...
\begin{definition}[Eq. 12 de \cite{Pottmann2009}]
  ...
  un bord $C^3$ par morceaux, et soit $x\in \dS$ une singularité. Alors, si
  ...
  boule $R$ et de l'angle $\alpha_0$, et :
  %
  \begin{align}
...
\end{definition}
%
$\FeatDD(R)$ nous donne un monôme d'exposant $-1$ auquel le coefficient est
...
\begin{center}
  ...
  CubeSphere » en fonction du rayon de la boule (décroissant, de droite
...
la surface, quand nous utilisons le rayon de la boule $R$ comme paramètre
d'espace d'échelle. Lorsque le rayon $R$ décroît sur une surface lisse,
l'estimateur de caractéristiques nous retourne la courbure au point $\vx$. Cette
...
%
Lorsque nous discrétisons $\BRShape$, pour un rayon de boule $R$ donné et un
pas de discrétisation $h$, une infinité de courbes euclidiennes
$\Boundary{\Shape}$ avec différentes valeurs de courbure au point $\vx$ donnent
...
(en noir) au point $\vx$ pour un rayon de boule $R$ (en orange) et un pas de
...
point $\vx$ qui correspondent à une zone plate à cause de la discrétisation. La
...
\begin{property}
  ...
  courbure maximale au point $\vx$ résultant à la même quantité $\FeatDD(R)$
  (\resp $\FeatTD(R)$) est :
  %
  \begin{equation}
...
inférieure à $\MeanCurvMax$ en 3D) pour un point $\vx$ de $\Boundary{\Shape}$,
nous ne pouvons décider si $\vx$ est sur une surface très légèrement lisse ou sur
une surface plate.
%
\\
%
...
\ref{fig:CubeSpherePlot_F_NoColor}, nous observons les trois cas en espace d'
...
digitale de $\FeatDD(R)$) au point $\vp \in \Boundary{\DigShape}$.
%
\\
%
...
(\RefFigure{fig:inversion} par exemple). S'il reste assez de
...
le comportement des trois distances lorsque le rayon de la boule change. En
...
\\
%
Finalement, pour un point $\vp$ de la forme $\DigShape \subset \Z^2$ ou $\Z^3$, nous
...
rayons de boule est en relié à la plus petite courbure d'une région \featsmooth
...
rayons. \todoJeremy{Essayer FeatureEstimator x Segments max}
%
...
\begin{table}[h]
  \begin{center}
    ...
    \begin{tabular}{@{}lrccc@{}}
      \toprule
      Méthode & \multicolumn{1}{r}{Paramètres} & \multicolumn{1}{r}{Segmentation} & \multicolumn{1}{r}{Espace d'Échelle} & \multicolumn{1}{r}{Robustesse}     \\ \midrule
...
lecture comme \emph{CubeSphere} ou \emph{SpheresUnion}.
%
\paragraph{}
%
\nauthor{Clarenz} offrent de très bons résultats. Comme espéré, les petits
...
\paragraph{}
%
\nauthor{Mérigot} offrent également de bons résultats sur des surfaces bruités
...
\paragraph{}
%
\nauthor{Mellado} semble quant à lui insensible au bruit, mais des artefacts
apparaissent sur les angles droits (voir les côtés de \emph{CubeSphere} par
exemple).
%
\paragraph{}
%
\nauthor{Pauly} fournissent de bons résultats mais ont tendances à considérer
...
\paragraph{}
%
\nauthor{Park} demandent beaucoup (cinq) paramètres pour être fonctionnel. De
...
souhaitons détecter. Autrement, d'après la \RefProperty{prop:minInterval}, notre
