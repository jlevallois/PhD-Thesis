...
\cleanchapterquote{[S'adressant à JD.] Ce n'est sans doute pas la réponse que tu
attendais Joséphine, mais comme toujours, je m'en fiche !}{Perry \textsc{Cox}
(\emph{John C. \textsc{McGinley}})}{Scrubs.}
...
%
L'avantage de notre classificateur est qu'il ne nécessite aucun paramètre
...
paramètre à choisir que l'ensemble de rayons. D'ailleurs, cet ensemble de rayons
constitue une limitation. Comme nous l'avons expliqué dans le
\RefSection{sec:applications:feature:II:kmax}, le rayon maximal de cet ensemble
est dépendant de la singularité maximale que nous souhaitons détecter, leur
réglage peut être important.


Enfin, comme pour les estimateurs de courbure, notre estimateur est basé sur
l'intégration volumique de la sphère. Cette opération reste coûteuse et peut
la rendre moins compétitive en terme de temps de calcul face à d'autres à
échelle fixe. Cependant, notre méthode permet de détecter les singularités à
plusieurs échelles ce qui est la principale défaillance des autres méthodes à
échelle fixe, et est robuste au bruit. \clearpage
