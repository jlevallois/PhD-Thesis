...
%
  \DMom{p_1 \cdots p_3}{h}(\DigF{\Ball{R}{\vx} \cap \Shape}{h},h) &= h^{3+\sigma}\Mom{p_1 \cdots p_3}\left
  ( (\frac{1}{h} \cdot \Ball{R}{\vx} \cap \Shape) \cap \Z^3 \right) \nonumber \\
  &= h^{3+{\sigma}} \Mom{p_1 \cdots p_3}\left( \frac{R}{h}\cdot(
  \Ball{1}{\frac{1}{R} \cdot \vx} \cap \frac{1}{R} \cdot \Shape) \cap \Z^3 \right) \nonumber \\
  &= R^{3+{\sigma}} \left( \frac{h}{R} \right)^{3+{\sigma}} \hspace{-0.3cm} \Mom{p_1 \cdots p_3} \left( \DigF{ \Ball{1}{\frac{1}{R}\cdot \vx}\cap\frac{1}{R}\cdot \Shape}{h/R} \right) \nonumber\\
  &= R^{3+{\sigma}}\DMom{p_1 \cdots p_3}{h}\left( \DigF{ \Ball{1}{\frac{1}{R}\cdot \vx}\cap\frac{1}{R}\cdot \Shape}{h/R}, {\frac{h}{R}} \right).
%
\end{align}
%
...
%
  \DMom{p_1 \cdots p_3}{h}(\DigF{\Ball{R}{\vx} \cap \Shape}{h},h) &= R^{3+\sigma}
  \Mom{p_1 \cdots p_3}\left( \Ball{1}{\frac{1}{R}\cdot \vx} \cap \frac{1}{R}\cdot \Shape
  \right) + R^{3+\sigma} O \left( \frac{h}{R} \right)^{\mu_{{\sigma}}}  \nonumber \\
   &= \Mom{p_1 \cdots p_3}( \Ball{R}{\vx} \cap \Shape ) + O( R^{3+{\sigma}-\mu_\sigma} h^{\mu_\sigma}).
%
\end{align}
%
...
%
  Pour tout point $\vx$, un rayon $R$ positif, pour tout vecteur $\vt$ avec pour
  norme $t \EqDef \| \vt \|_2 \le R$, nous avons :
%
  \begin{align}
    \left| \Mom{000}(\Ball{R}{\vx + \vt}) - \Mom{000}(\Ball{R}{\vx}) \right|
    & \le O(t R^2), \label{eq:zero-moments-ball-position-error} \\
    \left| \Mom{100}(\Ball{R}{\vx + \vt}) - \Mom{100}(\Ball{R}{\vx}) \right|
    & \le O(t R^3) + O( \|\vx\|_\infty t R^2 ), \label{eq:first-moments-ball-position-error}\\
    \left| \Mom{200}(\Ball{R}{\vx + \vt}) - \Mom{200}(\Ball{R}{\vx}) \right|
...
%
\begin{align}
  \Mom{p_1 \cdots p_3}&(\Ball{R}{\vx+\vt} \cap \Shape ) - \Mom{p_1 \cdots p_3}(\Ball{R}{\vx} \cap \Shape ) \\
    &= \Mom{p_1 \cdots p_3}( (\Ball{R}{\vx+\vt} \setminus \Ball{R}{\vx}) \cap \Shape ) \\
    & \quad - \Mom{p_1 \cdots p_3}( (\Ball{R}{\vx} \setminus \Ball{R}{\vx+\vt}) \cap \Shape )
\end{align}
%
Comme :
%
\begin{equation}
  \emptyset \neq Y_1 \subset Y_2 \subset \R^3 \Implies \sup_{ Y \subset Y_1} \left | \Mom{p_1 \cdots p_3}(Y) \right| \le \sup_{Y \subset Y_2} \left| \Mom{p_1 \cdots p_3}(Y)\right |,
\end{equation}
%
et avec la propriété d'additivité des intégrales, nous avons :
%
\begin{align}
  |\Delta(\vt)| &= |\Mom{p_1 \cdots p_3}( (\Ball{R}{\vx+\vt} \ominus \Ball{R}{\vx}) \cap \Shape )| \\
  &\le \sup_{Y \subset (\Ball{R}{\vx+\vt} \ominus \Ball{R}{\vx}) \cap \Shape} |\Mom{p_1 \cdots p_3}(Y)|\\
  &\le \sup_{Y \subset \Ball{R}{\vx+\vt} \ominus \Ball{R}{\vx}} |\Mom{p_1 \cdots p_3}(Y)|\\
  &\le \sup_{Y \subset \Ball{R+t}{\vx} - \Ball{R-t}{\vx}} |\Mom{p_1 \cdots p_3}(Y)|.
\end{align}
%
Nous couvrons alors la différence entre les deux boules centrées en des points
différents ($\Ball{R}{\vx+\vt} \ominus \Ball{R}{\vx}$) par la différence entre
deux boules de même centre mais de rayons différents ($\Ball{R+t}{\vx} -
\Ball{R-t}{\vx}$). Ceci est représenté en bleu sur la
\RefFigure{fig:moments-perturb}, correspond à un anneau en 2D de largeur $2t$.
Il s'agit évidemment d'une borne supérieure épaisse, mais les ordres de
perturbations sont les mêmes. Alors, pour le moment d'ordre zéro, nous utilisons
le volume de la boule :
%
\begin{align}
  \sup_{Y \subset \Ball{R+t}{\vx} - \Ball{R-t}{\vx}} |\Mom{000}(Y)| &
    = \Mom{000}( \Ball{R+t}{\vx} - \Ball{R-t}{\vx} ) \nonumber \\
    &
...
\label{lem:moment-pos-error}
  For any measurable subset $\Shape \subset \R^3$ and any vector $\vt$ with
  norm $t \EqDef \| \vt \|_2 \le R$, we have for $0 \le p+q+s
  \EqDef {\sigma} \le 2$:
  \begin{align}
    m_{p,q,s}(\Ball{R}{x+\vt} \cap X ) & = m_{p,q,s}(\Ball{R}{x} \cap X ) \nonumber\\
    &+ \sum_{i=0}^{{\sigma}} O(\|x\|^i tR^{2+{\sigma}-i}).
    %% m_{1,0,0}(\Ball{R}{x+\vt} \cap X ) &= m_{1,0,0}(\Ball{R}{x} \cap X ) + O(R^3t)\\
    %% m_{2,0,0}(\Ball{R}{x+\vt} \cap X ) &= m_{2,0,0}(\Ball{R}{x} \cap X ) + O(R^4t)\\
    %% m_{1,1,0}(\Ball{R}{x+\vt} \cap X ) &= m_{2,0,0}(\Ball{R}{x} \cap X ) + O(R^4t).
  \end{align}
\end{lemma}
