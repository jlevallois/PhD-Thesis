...
%
  Soit $\Shape$ un domaine compact de $\R^d$, dont le bord est $C^3$ et peut
  être décomposé en nombre fini de parties monotones. Alors, il existe une
  constance $h_0$ telle que pour tout pas de discrétisation $0 < h < h_0$, pour
...
%
  Soit $\Shape$ un domaine compact de $\R^3$ dont le bord est $C^3$ et peut
  être décomposé en nombre fini de parties monotones. Alors, il existe une
  constance $h_0$ telle que pour tout pas de discrétisation $0 < h < h_0$, pour
  tout $\vx \in \R^3$, pour un rayon $R \ge h$, nous avons\footnote{$\mu_i \ge
  1$ d'après l'\RefEquation{eq:MomentsByCounting-conv}} :
%
  \begin{align}
...
%
  Soit $\Shape$ un domaine compact de $\R^3$ telle que son bord $\Bd{X}$ a une
  continuité $C^3$ et peut être décomposé en nombre fini de parties monotones.
...
\begin{proof}
  ...
  \RefEquation{eq:MomentsByCounting-conv}). En donnant pour valeur $R = kh^\alpha$, $k$ étant une constante, la valeur optimale pour $\alpha$ minimisant le plus les erreurs est $\frac{1}{3}$. Ainsi, nous obtenons :
  %
  \begin{equation}
