...
par intération. En effet, à la place de calculer la mesure de $B_R(\vx)$ dans
...
volume, pour ensuite la diffuser aux calculs de courbures.\\
%
...
\begin{equation}\label{eq-shift-B-R-error-bound-3D}
  | \VolC(\DigF{B_R(\vxH) \cap \Shape}{h}, h) - V_R(\vx) |  \le K_1 h^\beta R^{2-\beta} +  R^2 \pi \vt (1 + O(R^2) + O(\vt)).
\end{equation}
%
...
\begin{equation} \label{eq-shift-curvhat-error-bound-h-only}
  | \CurvH{R}(\DSh,\vxH,h) - \Curv(\Shape,\vx) | \le O(h^\alpha) + O(h^{\beta-\alpha(1+\beta)}) + O(h^{\alpha'-2\alpha})+ O(h^{\alpha'}) + O(h^{2\alpha'-2\alpha}).
\end{equation}
%
...
$\alpha'$.
%
Si $\alpha' \ge \frac{3 \beta}{1 + \beta}$, alors $\alpha = \frac{\beta}{1 +
