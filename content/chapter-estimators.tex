...
$\alpha \ge 3$. Cela n'est pas applicable directement à des surfaces digitales car la perturbation est trop loin par rapport à la surface et correspond plus à des \anglais{outliers}. De plus, les contraintes topologiques expliquées précédemment sont déterminantes pour la convergence : si elles ne sont pas satisfaitent, l'estimation de la courbure gaussienne ne converge pas.
%
...
osculateurs (\anglais{osculating jets}) de \cauthors{Cazals}{Cazals2005}.
%
\\
%
...
estimateur \cite{Coeurjolly_ChapEstimateur}. Lorsque la forme est discrétisée
sur une grille avec une résolution $h$ tendant vers zéro, la quantité estimée
devrait converger vers la valeur attendue.
%
\\
%
En dimension 2, plusieurs méthodes se démarquent. Nous allons les détailler.
%
\paragraph{Reconnaissance d'objets digitaux}
%
Nous allons nous interesser dans un premier temps aux méthodes reconnaissant des
objets digitaux sur le bord de l'objet, afin d'en extraire la courbure.


\textbf{Analyse de segments digitaux maximaux}\\
...
formelle). La courbure de la surface étant défini comme l'inverse du rayon du
cercle osculateur, il parait logique de s'intéresser à reconnaitre des arcs de
cercle digitaux sur la surface digitale de la forme.
%
\\
%
...
mapper tous les surfels vers le \MDCA le plus proche. Afin de déterminer quel
...
continue, strictement positif et borné supérieurement, avec $0 < b \le a <
\frac{1}{2}$ correpondant aux bornes inférieures et supérieurs des longueurs des
\MDCA (\respp $\Omega(h^\alpha)$ et $O(h^b)$).
...
\end{equation}
%
et l'estimateur de dérivés pour une fonction discrète $F$ est :
%
\begin{equation}
...
\begin{equation}
  %
  \CurvH{n}_{BC}(\vp) = \frac{D_n^2(\vp_1) \ast D_n(\vp_2) - D_n^2(\vp_2) \ast D_n(\vp_1)}{D_n(\vp_1)^2 + D_n(\vp_2)^2} \,,
  %
\end{equation}
%
avec $n = \lfloor h^{2(\alpha -3)/3} \rfloor$ et $\alpha \in ]0,1]$ un paramètre
associé au niveau de bruit (plus $\alpha$ est proche de $0$, plus l'estimateur
sera robuste au bruit).
%
\\
%
\cauthors{Esbelin}{Esbelin2011} annoncent une convergence asymptotique en
