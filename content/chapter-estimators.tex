...
courbures principales (\RefSection{sec:estimators:moments}) en 3D. Nous adapterons
...
sens où ce dernier approxime suffisamment précisement la surface continue sous-jascente).
%
\begin{figure}[ht]
    \begin{center}
...
sont sur la surface paramétrique suffisemment lisse $F(x,y) \in \R^3$ et
...
Cette approche n'est pas applicable directement à des surfaces digitales car la perturbation par rapport à la surface est a une distance trop grande et correspond plus à des \anglais{outliers} qu'à du bruit de discrétisation. De plus, les contraintes topologiques expliquées précédemment (surface paramétrique, sommets de valence $6$) sont déterminantes pour la convergence : si elles ne sont pas satisfaitent, l'estimation de la courbure gaussienne ne converge pas.
%
...
$\vp$.
%
\todoInlineJeremy{Mouai ... a finir là ...}
%
...
osculateurs (\anglais{osculating jets}) de \cauthors{Cazals}{Cazals2005}. Un jet est un développement de Taylor tronqué, et ces jets contiennent des
informations sur les quantités géométriques locales (normales, courbures, etc.).
Un $n$-jet osculateur est un développement de Taylor tronqué de la fonction de hauteur à l'ordre $n$.
...
\begin{figure}[ht]{
    \begin{center}
    \includegraphics[height=6cm]{images/Curvature/Hough_bad}
    \end{center}}
...
anisotropes. Les auteurs rectifient cela en proposant de calculer l'algorithmes
...
\\
%
\cauthors{Coeurjolly}{Coeurjolly_ChapEstimateur} on proposé un état de l'art complet pour l'estimation de tangeantes et de la courbure en dimension 2. Nous allons les détailler les méthodes les plus représentatives.
%
...
cercle osculateur, il parait logique de s'intéresser à reconnaitre des arcs de
...
aucune ambiguité ne peut apparaitre, chaque surfel est relié à son \MDCA le plus
...
\frac{1}{2}$ correpond aux bornes inférieures et supérieurs des longueurs des
...
méthode basées sur les polynomes de Taylor.
%
\\
%
Le principe de cette méthode est de faire ajuster un polynome sur les valeurs de
...
étant les coefficients du polynome $P(\vx)$. Ils introduisent alors la notion de
rugosité (\anglais{roughness}) comment une fonction, le $R$ minimum nécessaire
pour trouver un polynome s'ajustant sur les données de la fonction $F$ avec une
...
\begin{figure}[ht]{
    ...
    possible pour lequel il existe un polynome de degré 2 ou plus qui passe au
...
dans le voisinnage du point $\vx$. L'inconvénient de cette méthode est qu'elle
