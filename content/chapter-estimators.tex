...
%
  Soit $\Shape$ un domaine compact de $\R^3$ à reach positif, dont le bord est $C^3$ et peut
...
\begin{proof}
 ...
 \end{align}
  ...
  \end{proof}
%
\paragraph{Théorie de perturbation de matrices}
%
...
preuves de convergence asymtotiques, il est nécessaire de comprendre comment une
...
courbures principales sur des patches surfaciques plats ou sphériques.
%
\paragraph{Convergence asymptotique des estimateurs digitaux de courbures principales}
%
...
\begin{proof}
  %
  Nous allons prouver la convergence asymtotique pour l'estimateur digital de la
...
\label{thm:multigrid-conv-principal-directions-and-normal}
  ...
  directions principales de courbure $\PrincDir{1}$ and $\PrincDir{2}$ sur des
  formes $\Shape$ pour des pas de discrétisations $h$ et rayon $R$ donnant des
  courbures principales distinctes. L'estimateur digital des normales par
  intégration converge également vers la normale $\NormalDir$. Plus précisement,
...
%
\subsubsection{Résultats sur l'estimation de la courbure par intégration volumique de la sphère}
%
...
choisi $R=10$ pour toutes les images de la \RefFigure{fig:digital-II-octa}. La
carte des couleurs pour les estimateurs de coubure moyenne et gaussienne va du
bleu (valeur minimale) au jaune (valeur maximale).
%
\begin{figure}[ht]
\begin{center}
  \includegraphics[width=.34\linewidth]{images/Curvature/Octa256_Mean_R_10_0001}
  \includegraphics[width=.34\linewidth]{images/Curvature/Octa256_Gauss_R_10_0001}\\
  \includegraphics[width=.30\linewidth]{images/Curvature/octa-256-prindir1}
  \includegraphics[width=.30\linewidth]{images/Curvature/octa-256-prindir2}
  \includegraphics[width=.30\linewidth]{images/Curvature/octa-256-normal} \\
...
\end{center}\vspace{-0.5cm}
  \caption{Estimateurs différentiels digitaux par intégration sur une surface digitale (\textsc{OctaFlower} de dimension $256^3$) avec $R = 10$. \emph{De gauche à droite, de haut en bas}, courbure moyenne, courbure gaussienne, premières et secondes directions principales de courbure et normales (zoom sur la troisième ligne). \label{fig:digital-II-octa}}
\end{figure}
%
\todoInlineJeremy{vérifier si j'ai mis convexe partout}
%
\subsection{Estimation sans paramètre de la courbure par intégration volumique de la sphère}
%
...
\RefFigure{fig:digital-II-octa-scale} montre le resultat de l'estimation de la
...
%
%\subsubsection{Propriétés sur les segments maximaux}
%
\subsubsection{Estimateur digital sans paramètre de courbure 2D}
%
...
d'avoir une convergence asymptotique dela courbure. Nous allons exploiter les
...
\begin{definition}
  ...
  Pour une forme $\DigShape \subset \Z^2$, l'estimateur digital sans paramètre de courbure 2D $\CurvHS$ au pointel $\vp \in \BdZ{\DigShape}$ est défini comme :
  \begin{equation}
  \CurvHS( \DigShape, \vp) \EqDef \frac{3\pi}{2 \rho(\DigShape)} - \frac{3 A(\DigShape,\vp)}{{\rho(\DigShape)}^3}
  \end{equation}
  où $\rho(\DigShape) =L_D^2(\DigShape)$ and $A(\DigShape,\vp) = Card(\Ball{\rho(\DigShape)}{\vp} \cap \DigShape)$.
\end{definition}
%
...
\RefDefinition{def:digital-2d-curvature} est alors indépendant à l'echelle de la
forme.
%
\\
%
...
un canditat parfait comme paramètre de rayon $R$, puisqu'il suit
approximativement $\Theta(h^\frac{1}{3})$, pour obtenir un estimateur digital
sans paramètre de courbure.
%
\begin{theorem}[Multigrid convergence of curvature estimator $\CurvHS$]
\label{thm:curvature-estimator-2d-pf-conv}
  %
  Soit $\Shape$ une forme convexe de $\R^2$ telle que son bord $\Bd{\Shape}$ est
  $C^3$ et sa courbure bornée. Soit $\DigShape = \DSh$. Alors, il existe une
  constante positive $h_0$, nous avons :
  %
  \begin{align}
    &\forall 0 < h \le h_0, \quad \forall \vx \in \partial \Shape, \quad \forall \vp \in \BdZ{\DigShape} \text{~avec~} \| h\vp -\vx\|_\infty \le h, \nonumber\\
...
\begin{figure}[ht]
  ...
  \end{center}
  \caption[Illustration de la dépendence du paramètre d'échelle $h$.]
  %
  {Illustration de la dépendence du paramètre d'échelle $h$. L'objet euclidien
...
paragraphe suivant traîte cette question.
%
\subsubsection{Estimateur digital local sans paramètre de courbure 2D}
%
\todoInlineJeremy{-------------------------------}
% \todoInlineJeremy{-------------------------------}
% \todoInlineJeremy{-------------------------------}
...
\subsection{Comparaison des estimateurs de courbure}%
