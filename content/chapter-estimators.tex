...
courbures principales et de directions principales à partir des valeurs propres
et des vecteurs propres de la matrice de covariance digitale dans la
\RefSection{def:principal-curv-estimators}. Afin de pouvoir obtenir des preuves
de convergence asymtotiques, il est nécessaire de comprendre comment une erreur
sur une matrice peut perturber sa décomposition. C'est pour cela que nous allons
dans un premier temps présenter un résultat de la théorie de perturbation de
matrices \cite{Bauer:1960,stewart1990matrix,bhatia1997matrix}.
%
\\
%
...
directions principales et de la normale peuvent être également apportés. Les
détails sont expliqués dans \cite{ChapterIICurvature}. La preuve est un peu plus
compliquée que pour les courbures principales car des problèmes interviennent
aux points ombilic de la surface, \cad lorsque les deux courbures principales
sont égales. De ce fait, pour tous ces points, les deux valeurs propres de la
matrice de covariance coïncident et leurs vecteurs propres couvrent un plan.
...
\label{thm:multigrid-conv-principal-directions-and-normal}
  %
  Soit $\Shape$ un domaine compact de $\R^3$ tel que son bord $\Bd{X}$ a un
...
courbures, celles-ci doivent être orientés dans la même direction afin d'éviter
...
%
\subsubsection{Résultats de l'estimation de la courbure par intégration volumique de la sphère}
%
Nous allons maintenant montrer quelques résultats. Une comparaison expérimentale
complète sera proposée dans le \RefSection{sec:comparaison-courbure}.
%
\begin{figure}[ht]
\begin{center}
  \includegraphics[width=.34\linewidth]{images/Curvature/octa-256-mean}
  \includegraphics[width=.34\linewidth]{images/Curvature/octa-256-gaussian}\\
...
\end{center}\vspace{-0.5cm}
  \caption{Estimateurs différentiels digitaux par intégration sur une surface digitale (\textsc{OctaFlower} de dimension $256^3$). \emph{De gauche à droite, de haut en bas}, courbure moyenne, courbure gaussienne, premières directions principales, secondes directions principales et normales (zoom sur la troisième ligne). \label{fig:digital-II}}
\end{figure}
%
\todoInlineJeremy{-------------------------------}
% \todoInlineJeremy{-------------------------------}
