...
%
Comme nous l'avons vu dans le chapitre précédent (\RefSection{sec:geo-diff}),
une forme peut être décrite à partir de ses quantités différentielles comme la
courbure. Et nous parlons bien de décrire, puisque toutes ces quantités sont ce
qui la caractérise de manière unique. C'est pour cela que plusieurs champs de
recherches se tournent vers la manière de récupérer ces quantités en les
estimant à partir de données \todoJeremy{citer un survey}.
%
\\
%
Ces données discrètes sont principalement liées à la façon de capturer les
objets ou de son utilisation et peuvent prendre la forme de nuages de points, de
maillages triangulés, ou encore de données digitales. Les besoins des
estimateurs sont différents et très liés à l'usage de la quantité différentielle :
certains privilégerons la rapidité d'estimation en dépis de la qualité notamment
dans un enjeu d'utilisation en temps interactif ou réel, tandis que d'autres (et
c'est notre cas) préfèrerons garantir la qualité des résultats de l'estimation
en priorité.
...
plus justes possible. Il nous parait également important de connaitre le
comportement de l'estimateur si nous prenons le coût d'augmenter la résolution
...
notre estimation à l'aide de preuves théoriques et pratiques.
%
...
$T$ la triangulation de Delaunay de $\mathcal{P}$ restraint à $\Shape$, et
...
est un développement de Taylor tronqué, un $n$-jet est un développement de Taylor tronqué à l'ordre $n$.
L'intérêt de ces jets est qu'ils contiennent des informations sur les quantités géométriques locales (tangentes / normales, courbures
principales, etc.).
%
\\
%
...
$n$-jets. Il est à noter que le terme de jet osculateur désigne les $2$-jets. Ainsi, comme les $n$-jets
contiennent des propriétés différentielles de la courbe/surface de l'ordre $n$,
cela implique que toutes les quantités différentielles de l'ordre $n$ peuvent
être calculées grâce aux $n$-jets. Ce qui nous intéresse ici, ce sont les
informations reliées à la courbure contenues dans les jets osculateurs.
...
allons les détailler les méthodes les plus représentatives.
%
...
\cauthors{Coeurjolly}{Coeurjolly2001} ont montré qu'une façon simple d'estimer
...
longueur est en $\Theta(h^{-\frac{1}{3}} \log \left(\frac{1}{h}\right))$ en
moyenne (voir le \RefLemma{lem:law-length-MDSS}). Cette borne supérieure a pour
conséquence de réfuter la preuve de convergence avancée dans
\cite{Coeurjolly2001}.
%
...
\\
%
Le principe de l'estimateur est simple : pour une forme $\Shape \subset \R^2$ avec un champ de courbure continu,
...
\MDCA     & \svgYes & \svgNope & \svgNope & \svgNope & \svgNope & ??? & \svgNope & \cite{Roussillon2011} \\
\II       & \svgYes & \svgYes & \svgYes & \svgYes & \svgYes & Convexe $C^3$ à courbure non nulle avec $R = kh^\frac{1}{3}$ & $O(h^\frac{1}{3})$ & \cite{CVIU2014} \\
\hline
\end{tabular}
\end{table}
...
les cellules de Voronoï, plus particulièrement les méthodes de \VCMM
