...
(\RefSection{sec:estimators:volume}) afin d'en extraire des estimateurs de
courbure en 2D et courbure moyenne (\RefSection{sec:estimators:volume}) et
courbures principales (\RefSection{sec:ii-3d}) en 3D. Nous adapterons
ces estimateurs à des versions ne nécessitant aucun paramètre (l'unique paramètre
étant la taille de la sphère d'intégration) dans la
\RefSection{sec:curvature:parameter-free}. Enfin, nous comparerons nos
...
\todoInlineJeremy{Introduction curvature à faire}
%
...
approche qui essaye de plaquer les perturbations sur la surface à travers un
moyennage.
%
...
sont sur la surface paramétrique suffisamment lisse $F(x,y) \in \R^3$ et
...
perturbation par rapport à la surface est a une distance trop grande et
correspond plus à des \anglais{outliers} qu'à du bruit de discrétisation. De
...
consécutifs à $\vp$  (voir \RefFigure{fig:normal-cycle-1}), et $|\beta(\ve)|$
est l'angle entre les normales des triangles de $\Boundary V$ incidents au point
$\vp$ (voir \RefFigure{fig:normal-cycle-2}). Si $\ve$ est convexe, le signe de
$\beta(\ve)$ doit être positif et négatif si $\ve$ est concave.
%
\missingfigure{fig:normal-cycle-1}
\missingfigure{fig:normal-cycle-2}
...
$T$ la triangulation de Delaunay de $\mathcal{P}$ restraint à
$\Shape$, et $\epsilon$ la densité de l'échantillonnage de la surface. Si
$\mathcal{P}$ est le $\epsilon$-échantillonnage de $\dS$, \cad si la boule
$\Ball{\epsilon lfs(\vx)}{\vx}$ centrée au point $\vx$ et de rayon $lfs(\vx)$
rencontre $\mathcal{P}$. $lfs(\vx)$ (pour \anglais{local feature size})
correspond à la distance entre $\vx$ et l'axe médian de $\Shape$. Cela signifie
pour résumer que l'échantillonnage doit être plus dense sur les zones à forte
courbure. Alors, lorsque $\epsilon < 0.08$
%
\begin{align}
...
sur $\Boundary T$ et est un homéomorphe de $\Boundary T$ vers $M$ ).
...
dépendant de la taille de la boule et du paramètre d'échantillonnage $\epsilon$. Nos travaux sur les
...
est un développement de Taylor tronqué, et ces jets contiennent des informations
sur les quantités géométriques locales (tangentes / normales, courbures
principales, etc.). Un $n$-jet est un développement de Taylor tronqué de la
fonction de hauteur à l'ordre $n$.
%
\\
%
Le développement de Taylor d'une courbe à l'ordre $n$ équivaut à :
%
\begin{align}
...
$n$-jets. Le terme de jet osculateur désigne les $2$-jets. Ainsi, comme les $n$-jets
contiennent des propriétés différentielles de la courbe/surface de l'ordre $n$,
ce qui implique que toutes les quantités différentielles de l'ordre $n$ peuvent
être calculées grâce aux $n$-jets. Ce qui nous intéresse ici, ce sont les
informations reliées à la courbure contenues dans les $2$-jets.


Le principe est simple : (1) collecter les $N$ voisins du point $\vp$, (2) résoudre l'ajustement polynomial, (3) récupérer les quantités différentielles.
%
\\
%
...
\\
%
Pour la seconde étape, il faut effectuer une analyse en composantes principales
(\ACP) de notre ensemble de points voisins afin d'estimer grossièrement la
normale au point $\vp$. Cela nous permet de placer le point $\vp$ à l'origine du repère et de changer de système de coordonnées pour celui de Monge (l'axe $\vec{z}$ est aligné avec la normale, et les axes $\vec{x} et \vec{z}$ sont alignés avec les directions principales de courbure). Alors :
%
\begin{align}
...
\end{align}
%
où $\PrincCurv{1}$ et $\PrincCurv{2}$ sont les courbures principales, $b_0$ et $b_3$ sont les dérivées directionelles de $\PrincCurv{1}$ et $\PrincCurv{2}$ le long de leur ligne de courbure respective, et are the directional derivatives of k1,k2 along their respective curvature line, while b1,b2 are the directional derivatives of k1,k2 along the other curvature lines.

Alors, en ayant recours à de l'ajustement de polynômes, nous pouvons récupérer les $n$-jets de la surface dans ce système de coordonnées. Nous calculons alors la base de Monge $(d_1, d_2, n)$
...
%
\paragraph{Mesure de covariance des cellules de Voronoi}
%
Une autre famille de techniques utilise le diagramme de Voronoi
\cite{Alliez2007,Merigot2009,Merigot2011,Cuel2014DGCI} principalement pour estimer les normales, mais peuvent également extraire des informations de courbure. L'idée principale
...
\cauthors{Mérigot}{Merigot2011} proposent alors d'associer en chaque point $\vp$
...
\begin{figure}[ht]{
    ...
    \end{center}}
    \caption[Principe de \VCM.]{Principe de \VCM (Figure~2 de \cite{Merigot2011}). En noir : séparation des cellules de Voronoi, en orange : \comJeremy{longueur des directions des cellules} de Voronoi.
...
cette approche donne des résultats comparables à \JetFitting pour les
courbures. \cauthors{Cuel}{Cuel2014} propose une comparaison complète pour l'estimation des normales.
%
...
détecter des formes paramétriques simples comme des lignes ou des arcs de cercle \cite{Duda1972}. Cependant, elle peut être étendu en dimension 3 : l'accumulateur n'est plus une image, mais un objet 3D. \cauthors{Boulch}{Boulch2012} utilisent une boule accumulatrice (voir la \RefFigure{fig:hough-ball}) de $M$ bins.
%
\begin{figure}[ht]{
    \begin{center}
...
\\
%
Alors, les auteurs découpent le nuage de points en ensemble de plans (définis par trois points) et normales. Si le plan est loin d'un coin ou d'une singularité, alors la normale est correct. Cependant, si ce plan est près d'un coin, la normale est perturbée par cette singularité (voir la \RefFigure{fig:Hough_bad}-gauche).
%
\begin{figure}[ht]{
    \begin{center}
...
\RefFigure{fig:Hough_bad}-droite).
%
\\
%
...
\\
%
\cauthors{Coeurjolly}{Coeurjolly_ChapEstimateur} on proposé un état de l'art complet pour l'estimation de tangentes et de la courbure en dimension 2. Nous allons les détailler les méthodes les plus représentatives.
%
...
objets digitaux sur le bord de l'objet, afin d'en extraire des informations différentielles.
...
des segments maximaux digitaux que nous avons défini dans le chapitre précédant
...
de l'estimateur reposant sur celle-ci. Nous verrons dans la partie comparative
que cet estimateur ne converge pas.


\textbf{Analyse des arcs de cercle digitaux maximaux}\\
...
ils calculent les $(A_i)_{i \in 1,n}$ \MDCA ($n$ étant le nombre totale de
\MDCA) du bord digital $\BdZ{\DigShape}$ avec $\DigShape = \DSh$. Ils calculent
également les $(V_i)_{i \in 1,n}$, une partition du bord digital permettant de
faire correspondre tous les surfels vers le \MDCA le plus proche. Afin de déterminer quel
est le \MDCA le plus proche d'un surfel, il faut que la distance vers le surfel
au centre\footnote{le surfel le plus proche au niveau de la moitié de sa
longueur digitale} du \MDCA soit le plus petit par rapport aux autres. Ainsi,
aucune ambiguïté ne peut apparaître, chaque surfel est relié à son \MDCA le plus
...
\MDCA (\respp $\Omega(h^a)$ et $O(h^b)$). Cette dernière hypothèse n'est pas prouvé à ce jour.


Le principal avantage de ces deux méthodes est qu'ils ne nécessitent aucun
paramètre. \MDCA a l'avantage supplémentaire qu'une preuve de convergence
asymptotique est avancée, contrairement à \MDSS. Cependant, ces méthodes de
reconnaissances d'objets sont assez sensibles lorsque les données sont bruités.
C'est ce que nous verrons dans la section comparative
(\RefSection{sec:comparaison-courbure}).
%
...
\begin{equation}
  %
  \CurvH{n}_{BC}(\vp) \EqDef \frac{D_n^2(\vp_1) \ast D_n(\vp_2) - D_n^2(\vp_2) \ast D_n(\vp_1)}{D_n(\vp_1)^2 + D_n(\vp_2)^2} \,.
  %
\end{equation}
...
$O(h^{{2/3}^k})$ des dérivées du $k$-ième ordre pour des formes $C^3$ (donc en
...
%
\cauthors{Provot}{Provot2011} proposent quant à eux à s'intéresser aux
...
\ComputationalGeometry. Cette approche permet d'approcher les $k$ dérivées de
...
étant les coefficients du polynôme $P(\vx)$. Ils introduisent alors la notion de
rugosité (\anglais{roughness}) comment une fonction, le $R$ minimum nécessaire
...
dépend énormément du paramètre de rugosité maximale. De plus, ils ne fournissent
...
dimension 2, mais qui peuvent être sensibles au bruit de la forme ou dont la
preuve de convergence repose sur les paramètres choisis. De plus pour
l'estimation de courbure le long de courbes 2D, la convergence asymptotique
d'estimateurs ne nécessitant aucun paramètre représente toujours un défi (des
...
panorama complet. L'idée principale de cette approche est de déplacer un support
...
\RefSection{sec:notions:digital-shapes}), l'implémentation de ces estimateurs
...
\end{align}
%
Ainsi, l'\RefEquation{eq:MomentsOnUnitaryBall-conv} est le résultat de la
...
%
\subsection{Analyse de la convergence asymptotique}
%
...
%
\todoJeremy{Je ne sais pas trop quoi dire pour les version locales $l_\infty$}
%
...
espérer une robustesse aux bruit et aux données aberrantes (\anglais{outliers}).
...
\RefFigure{fig:curv-experiments-ellipse-noise}. Notre estimateur $\CurvH{R}$
...
Ces estimateurs souffrent par contre de certaines limitations.\\
%
...
Les perspectives sont multiples pour ces travaux.\\
%
