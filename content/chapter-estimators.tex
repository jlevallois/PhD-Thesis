...
courbure 2D et courbure moyenne 3D (\RefSection{sec:estimators:volume}) et
courbures principales 3D (\RefSection{sec:estimators:moments}). Nous adapterons
...
\anglais{outliers}). \todoJeremy{détailler plus en détail ($>\_<$), avec formules et
...
panorama complète. L'idée principale de cette approche est de déplacer un noyau
sur les points $\vx$ sur le bord $\dS$ d'une forme $\Shape$ et calculer
l'intégrale de l'intersection entre $\Shape$ et le noyau. Différents noyaux
peuvent être considérés (sphère euclidienne, boule euclidienne) ainsi que
...
\begin{definition}
  ...
  et $\chi(\cdot)$ est la fonction caractéristique de $\Shape$. En dimension 2,
  nous noterons cette quantité $A_R(\vx)$.
  %
\end{definition}
%
...
(\cad les barycentres de linels en 2D ou surfels en 3D), calculer la quantité
(aire, volume ou matrice de covariance) et enfin estimer l'information de
courbure $\CurvT{R}$, $\MeanCurvT{R}$, $\PrincCurvT{1}{R}$ ou
$\PrincCurvT{2}{R}$.
...
\end{align}
%
Alors, si nous pré-calculons tous les noyaux de déplacement
...
avoir des calculs dépendant de la taille du noyau mais linéaires à la taille de
