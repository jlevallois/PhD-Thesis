...
\begin{figure}[ht]
  \begin{center}
    \input{figures/tikz/RingNeighborhood.tikz}
  \end{center}
  [$1$-voisinage et $2$-voisinage.]{$1$-voisinage de valence $6$ et
  $2$-voisinage de valence $5$.}
  \label{fig:one-ring-neighborhood-2}
\end{figure}
%
...
très couteux en temps. Nous proposons de limiter le nombre de rayons locaux à
$K$ rayons. Cela aura pour conséquence la possibilité de précalculer les boules
...
optimisés lorsque nous nous déplaçons sur des élements de surface adjacents avec
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}llllrr@{}}
    \toprule
    Forme   & Équation (parametrique en 2D, implicite en 3D) & Paramètres & Domaine    & $k_{min}$ & $k_{max}$ \\ \midrule
...
les estimateurs digitaux de courbure 2D $\CurvH{R}$, de courbure moyenne 3D
$\MeanCurvH{R}$ et de courbures principales $\PrincCurvH{1}{R}$ et
$\PrincCurvH{2}{R}$ par intégration. La première ligne des
...
\begin{figure}[ht]
  \begin{center}
    ...
    \caption[Évaluation expérimentale sur l'objet \Ellipse.]{
      ...
      $l_\infty$ (\emph{à droite}) d'estimation de courbure 2D de $\CurvH{R}$
      ...
      et $l_\infty$ (\emph{à droite}) d'estimation de courbure 2D de $\CurvHS$,
...
\end{figure}
%
Alors, expérimentallement, le choix de $\alpha = \frac{1}{3}$ est cohérent pour
obtenir une vitesse de convergence des estimateurs $\CurvH{R}$, $\MeanCurvH{R}$,
$\PrincCurvH{1}{R}$, $\PrincCurvH{2}{R}$ en $O(h^\frac{1}{3})$.
%
\paragraph{Convergence expérimentale des estimateurs digitaux de courbure 2D}
%
...
$\CurvHS$ et $\CurvH{R}$, grâce à la nature adaptive de cet estimateur. Nous
...
%
\paragraph{Convergence expérimentale des estimateurs digitaux de courbure 3D}
%
...
peut être judiscieux de connaître le comportement d'un estimateur face à des
...
\end{align}
%
Alors, si nous précalculons tous les noyaux de déplacement
...
que les sous-noyaux $\DigF{\Ball{R}{\mathbf{0} \pm \vec{\delta}} \setminus
\Ball{R}{\mathbf{0}}}{h}$.
...
\begin{figure}[ht]
  \begin{center}
    ...
    \caption{
      ...
      estimateurs de courbure 2D sur l'objet \Flower (\emph{en haut à gauche})
      et des estimateurs de courbure moyenne 3D sur l'objet \RoundedCube
      (\emph{en bas}). Temps de calcul en secondes pour les estimateurs de
      courbure 2D sans paramètre sur l'objet \Ellipse (\emph{en haut à droite}).
...
espérer une robustesse aux bruit et aux données abbérantes (\anglais{outliers}).
...
\end{figure}
%
Nous expérimentons nos estimateurs de courbures 2D et 3D avec des objets bruités
dans les
\RefFigureTD{fig:curv-experiments-ellipse-noise}{fig:curv-experiments-rounded-noise}
...
(\II) et \BC sont extrèmement robuste au bruit lorsqu'il n'est pas trop grand
($t < 0.5$). Nous pouvons d'ailleurs observer que notre estimateur semble plus
robuste que l'estimateur \BC. En dimension 3D, nous pouvons constater que nos
...
produit des statistiques robustes.



, examples of noisified objects are
given. Our experimental setting can be described as follows: for both the flower
in 2D and the rounded cube in 3D ($h=0.1$ for both), we slightly increase the
noise parameter from 0 to 1 and we plot the $l_\infty$ error. In dimension two
(Fig.~\ref{fig:experimentsNoise}-$(e)$), we observe that estimators based on
geometrical object recognition (MDSS and MDCA) are highly sensitive to contour
perturbations. Both BC and II are extremely robust to small noise but for
$\beta$ greater than 0.5, II significantly outperforms BC. In dimension 3, we
observe that both II and Jet Fitting approaches lead to quite stable results.
Indeed, some computations in the Jet Fitting approach rely on a point-set PCA
(Principal Component Analysis) which also provides robust statistics.
%
\section{Conclusion}
...
% \todoInlineJeremy{-------------------------------}
% \todoInlineJeremy{-------------------------------}
\todoInlineJeremy{Conclusion estimators à faire}
\todoInlineJeremy{parler du bruit à un moment}
%
%
% \begin{figure}[ht]
%   \begin{center}
...
%       \multicolumn{4}{c}{\textbf{Différents $\alpha$ (Erreur $l_\infty$)}}
%       \\ \toprule
%       &
%       \textsc{Ellipse} &
%       \textsc{Flower} &
...
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\CurvH{R}$} &
%       \includegraphics[width=4.5cm]{graphs/Ellipse_ALPHA_Loo} &
%       \includegraphics[width=4.5cm]{graphs/Flower_ALPHA_Loo} &
%       \includegraphics[width=4.5cm]{graphs/AccFlower_ALPHA_Loo}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des convergences asymptotiques de l'estimateur $\CurvH{R}$
%       avec différentes valeurs de $\alpha$ pour le rayon $R$ ($R=kh^\alpha$) sur
%       l'objet \Ellipse, \Flower et \AccFlower. L'erreur est en $l_\infty$.
%       %
%       }
%       \label{fig:curv-experiments2D-alpha}
%   \end{center}
% \end{figure}
%
% \begin{figure}[ht]
...
%     \setlength{\tabcolsep}{0.0pt}
%     \begin{tabular}{@{}l c c c@{}}
%       \multicolumn{4}{c}{\textbf{Comparaison (Erreur $l_\infty$)}}
%       \\ \toprule
%       &
%       \textsc{Ellipse} &
%       \textsc{Flower} &
...
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\Curv$} &
%       \includegraphics[width=4.5cm]{graphs/Ellipse_Loo} &
%       \includegraphics[width=4.5cm]{graphs/Flower_Loo} &
%       \includegraphics[width=4.5cm]{graphs/AccFlower_Loo}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des erreurs $l_\infty$ d'estimation de courbure 2D de
%       $\CurvH{R}$ (\II) avec les estimateurs \BC \cite{Esbelin2011}, \MDSS
%       \cite{Coeurjolly2001,deVieilleville2007} et \MDCA \cite{Roussillon2011}
%       sur l'objet \Ellipse, \Flower et \AccFlower.
%       %
%       }
%       \label{fig:curv-experiments2D-loo}
%   \end{center}
% \end{figure}
%
% \begin{figure}[ht]
...
%     \setlength{\tabcolsep}{0.0pt}
%     \begin{tabular}{@{}l c c c@{}}
%       \multicolumn{4}{c}{\textbf{Comparaison (Erreur $l_2$)}}
%       \\ \toprule
%       &
%       \textsc{Ellipse} &
%       \textsc{Flower} &
...
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\Curv$} &
%       \includegraphics[width=4.5cm]{graphs/Ellipse_L2} &
%       \includegraphics[width=4.5cm]{graphs/Flower_L2} &
%       \includegraphics[width=4.5cm]{graphs/AccFlower_L2}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des erreurs $l_2$ d'estimation de courbure 2D de $\CurvH{R}$
%       (\II) avec les estimateurs \BC \cite{Esbelin2011}, \MDSS
%       \cite{Coeurjolly2001,deVieilleville2007} et \MDCA \cite{Roussillon2011}
%       sur l'objet \Ellipse, \Flower et \AccFlower.
%       %
%       }
%       \label{fig:curv-experiments2D-l2}
%   \end{center}
% \end{figure}
%
% \begin{figure}[ht]
...
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\MeanCurvH{R}$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_ALPHA_Mean_Loo} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_ALPHA_Mean_Loo} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_ALPHA_Mean_Loo}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\PrincCurvH{R}{1}$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_ALPHA_k1_Loo} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_ALPHA_k1_Loo} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_ALPHA_k1_Loo}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\PrincCurvH{R}{2}$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_ALPHA_k2_Loo} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_ALPHA_k2_Loo} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_ALPHA_k2_Loo}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des convergences asymptotiques des estimateurs $\MeanCurvH{R}$
%       (\emph{première ligne}), $\PrincCurvH{1}{R}$ (\emph{seconde ligne}) et
%       $\PrincCurvH{2}{R}$ (\emph{troisième ligne}) avec différentes valeurs de
%       $\alpha$ pour le rayon $R$ ($R=kh^\alpha$) sur l'objet \Sphere,
%       \RoundedCube et \Goursat. L'erreur est en $l_\infty$.
%       %
%       }
%       \label{fig:curv-experiments3D-alpha}
%   \end{center}
% \end{figure}
% %
% \begin{figure}[ht]
...
%       Surface de \textsc{Goursat}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\MeanCurv$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_Mean_Loo} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_Mean_Loo} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_Mean_Loo}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\PrincCurv{1}$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_k1_Loo} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_k1_Loo} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_k1_Loo}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\PrincCurv{2}$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_k2_Loo} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_k2_Loo} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_k2_Loo}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des convergences asymptotiques des estimateurs $\MeanCurvH{R}$
%       (\emph{première ligne}), $\PrincCurvH{1}{R}$ (\emph{seconde ligne}) et
%       $\PrincCurvH{2}{R}$ (\emph{troisième ligne}) avec \JetFitting
%       \cite{Cazals2005} sur l'objet \Sphere, \RoundedCube et \Goursat. L'erreur
%       est en $l_\infty$.
%       %
%       }
%       \label{fig:curv-experiments3D}
%   \end{center}
% \end{figure}
% %
% \begin{figure}[ht]
...
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\MeanCurv$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_Mean_L2} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_Mean_L2} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_Mean_L2}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\PrincCurv{1}$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_k1_L2} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_k1_L2} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_k1_L2}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\PrincCurv{2}$} &
%       \includegraphics[width=4.5cm]{graphs/Sphere_k2_L2} &
%       \includegraphics[width=4.5cm]{graphs/RoundedCube2_k2_L2} &
%       \includegraphics[width=4.5cm]{graphs/BlobbyCube_k2_L2}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des convergences asymptotiques des estimateurs $\MeanCurvH{R}$
...
%
%
% \subsection{Estimateurs sans paramètres}
%
%
% \begin{figure}[ht]
%   \begin{center}
%     \setlength{\tabcolsep}{0.0pt}
%     \begin{tabular}{@{}l c c@{}}
%       \multicolumn{3}{c}{\textbf{Comparaison (Erreur $l_\infty$)}}
%       \\ \toprule
%       &
%       \textsc{Ellipse} &
%       \textsc{Flower}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\Curv$} &
%       \includegraphics[width=4.5cm]{graphs/Ellipse_PF_Loo} &
%       \includegraphics[width=4.5cm]{graphs/Flower_PF_Loo}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des erreurs $l_\infty$ d'estimation de courbure 2D de
%       $\CurvH{R}$, $\CurvHS$ et $\CurvHSL$ avec l'estimateur \MDCA
%       \cite{Roussillon2011} sur l'objet \Ellipse et \Flower.
%       %
%       }
%       \label{fig:curv-experiments2D-pf-loo}
%   \end{center}
% \end{figure}
%
% \begin{figure}[ht]
...
%     \setlength{\tabcolsep}{0.0pt}
%     \begin{tabular}{@{}l c c@{}}
%       \multicolumn{3}{c}{\textbf{Comparaison (Erreur $l_2$)}}
%       \\ \toprule
%       &
%       \textsc{Ellipse} &
%       \textsc{Flower}
%       \\
%       \rotatebox{90}{~~~~~~~~~~~~~~$\Curv$} &
%       \includegraphics[width=4.5cm]{graphs/Ellipse_PF_L2} &
%       \includegraphics[width=4.5cm]{graphs/Flower_PF_L2}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des erreurs $l_2$ d'estimation de courbure 2D de $\CurvH{R}$,
%       $\CurvHS$, $\CurvHSL$, $\CurvHSLK{5}$, $\CurvHSLK{10}$ et $\CurvHSLK{50}$
%       avec l'estimateur \MDCA \cite{Roussillon2011} sur l'objet \Ellipse et
%       \Flower.
%       %
%       }
%       \label{fig:curv-experiments2D-pf-l2}
%   \end{center}
% \end{figure}
%
% \begin{figure}[ht]
...
%       $\PrincCurv{2}$
%       \\
%       \rotatebox{90}{~~~~~~~\textsc{Ellipsoid}} &
%       \includegraphics[width=4.5cm]{graphs/Ellipsoid_PF_Mean_Loo} &
%       \includegraphics[width=4.5cm]{graphs/Ellipsoid_PF_k1_Loo} &
%       \includegraphics[width=4.5cm]{graphs/Ellipsoid_PF_k2_Loo}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des erreurs $l_\infty$ d'estimation de courbure moyenne de
%       $\MeanCurvHS$, $\MeanCurvHSL$, $\MeanCurvHSLK{5}$, $\MeanCurvHSLK{10}$ et
%       $\MeanCurvHSLK{50}$ (\emph{première colonne}), de première courbure
%       principale de $\PrincCurvHS{1}$, $\PrincCurvHSL{1}$,
%       $\PrincCurvHSLK{1}{5}$, $\PrincCurvHSLK{1}{10}$ et $\PrincCurvHSLK{1}{50}$
%       (\emph{seconde colonne}) et de seconde courbure principale de
%       $\PrincCurvHS{2}$, $\PrincCurvHSL{2}$, $\PrincCurvHSLK{2}{5}$,
%       $\PrincCurvHSLK{2}{10}$ et $\PrincCurvHSLK{2}{50}$ (\emph{troisième
%       colonne}) sur l'objet \Ellipsoid.
%       %
%       }
%       \label{fig:curv-experiments3D-pf-loo}
%   \end{center}
% \end{figure}
% %
% \begin{figure}[ht]
...
%       \\
%       \rotatebox{90}{~~~~~~~\textsc{Ellipsoid}} &
%       \includegraphics[width=4.5cm]{graphs/Ellipsoid_PF_Mean_L2} &
%       \includegraphics[width=4.5cm]{graphs/Ellipsoid_PF_k1_L2} &
%       \includegraphics[width=4.5cm]{graphs/Ellipsoid_PF_k2_L2}
%     \end{tabular}
%     \caption{
%       %
%       Comparaison des erreurs $l_2$ d'estimation de courbure moyenne de
%       $\MeanCurvHS$, $\MeanCurvHSL$, $\MeanCurvHSLK{5}$, $\MeanCurvHSLK{10}$ et
%       $\MeanCurvHSLK{50}$ (\emph{première colonne}), de première courbure
%       principale de $\PrincCurvHS{1}$, $\PrincCurvHSL{1}$,
%       $\PrincCurvHSLK{1}{5}$, $\PrincCurvHSLK{1}{10}$ et $\PrincCurvHSLK{1}{50}$
...
%       %
%       }
%       \label{fig:curv-experiments3D-pf-l2}
%   \end{center}
% \end{figure}
%
%%%%%%%%%%%%%%%%%%%
