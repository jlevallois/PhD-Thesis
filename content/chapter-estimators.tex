...
\begin{definition}
  ...
  courbure moyenne sans paramètres $\MeanCurvHS$ au pointel $\vp \in
...
\begin{definition}
  %
  Soit $Z$ be a digital shape in $\Z^3$, we define the {\em parameter-free principal
  curvature estimators} $\hat{\kappa}^{1*}$ and
  $\hat{\kappa}^{2*}$ of $Z$ at point $p \in \BdZ{Z}$ as
  %
  \begin{align}
      %
...
le plus long et de sa borne $h\rho'(\DigShape, \vp) \in \Theta(1)$. Dans ces conditions, nous ne pouvons rien espérer en terme de convergence asymptotique. Mais à nouveau, expérimentalement, ce mauvais comportement n'est pas observé (voir \RefSection{sec:comparaison-courbure}).
%
\todoInlineJeremy{-------------------------------}
% \todoInlineJeremy{-------------------------------}
% \todoInlineJeremy{-------------------------------}
