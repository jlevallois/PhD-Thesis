...
%
\subsubsection{Estimateur digital sans paramètre de courbures 3D}
%
...
\label{prop:slices-3d}
  ...
  $\vec{\ve}\in\{ \vec{e}_1, \vec{e}_2, \vec{e}_3 \}$).
  %
  \\
  %
  Soit $\dS_{\vec{\ve}}(\vx)$ l'ensemble de l'intersection $\Shape
  \cap \pi_{\vec{\ve}}(\vx)$. Alors, il y a \textbf{au moins} deux ensembles
  $\dS_{\vec{e}_1}(\vx)$, $\dS_{\vec{e}_2}(\vx)$ ou
  $\dS_{\vec{e}_3}(\vx)$ sont localement des courbes auxquelles la
...
\begin{proof}
  ...
  \begin{figure}[ht]
    \begin{center}
      \begin{overpic}[width=8cm]{figures/Slice_3D}
      \put(40.5,48){$\pi_{\vec{e}_2}$}
      \put(95,50){$\pi_{\vec{e}_2}$}
      ...
      \put(52,48){$\textcolor{gray}{\vec{e}_1}$}
      \put(51.5,44){$\textcolor{gray}{\vec{e}_2}$}
      \put(47,51.5){$\textcolor{gray}{\vec{e}_3}$}
      \end{overpic}
    \end{center}
    %
  ...
  \end{figure}
  %
  Tout d'abord, si un des plans $\pi_{\vec{e}_1}$, $\pi_{\vec{e}_2}$ ou
  $\pi_{\vec{e}_3}$ est le plan tangent à $\vx$, les deux autres sont des plans
  normaux de $\dS$ au point $\vx$, \cad qu'ils contiennent la
  normale $\vec{\NormalDir}$. Dans ce cas, le \comJeremy{théorème d'Euler} nous
  dit que toutes les courbes définies par l'intersection entre le plan normal et
  la surface $\dS$  ont une courbure $\Curv$ égale à $\PrincCurv{1}
  \cos^2 \theta + \PrincCurv{2} \sin^2 \theta$, $\theta$ étant un angle. Il est
  alors immédiat que $|\Curv|$ est contenu entre $\lbrack
  \min(|\PrincCurv{1}|,|\PrincCurv{2}|), \max(|\PrincCurv{1}|,|\PrincCurv{2}|)
  \rbrack$, et est de ce fait borné par $K$ sur ces deux plans.
  %
  Ensuite, pour tout $\vec{\ve} \in \{ \vec{e}_1, \vec{e}_2, \vec{e}_3 \}$,
  l'ensemble $\dS_{\vec{\ve}}$ est localement une courbe 3D qui croise $\vx$ sur
  la surface $\dS$. Il y a au moins un $\vec{\ve} \in \{ \vec{e}_1, \vec{e}_2,
  \vec{e}_3 \}$ dont $\vec{\NormalDir} \cdot \vec{\ve} \ge \frac{\sqrt{2}}{2}$.
  En effet, notons la normale $\vec{\NormalDir} = a \vec{e}_1 + b \vec{e}_2 + c
  \vec{e}_3$ et par exemple $\vec{\NormalDir} \cdot \vec{e}_1 \ge
  \frac{\sqrt{2}}{2}$, alors $b^2 + c^2 = 1 - a^2 = 1 - (\vec{\NormalDir} \cdot
  \vec{e}_1)^2 < \frac{1}{2}$.
  %
\end{proof}
%
