...
\begin{definition}\label{def:Volume}
  ...
  \begin{equation}
    V_R(\vx) \EqDef \int_{B_R(\vx)}  \chi(p)dp\,,
  \end{equation}
  où $B_R(\vx)$ est une boule euclidienne de rayon $R$ et de centre $\vx$, et
...
par intégration. En effet, à la place de calculer la mesure de $B_R(\vx)$ dans
...
pqs$-moments $\Mom{pqs}(Y)$ de $Y$ :
\begin{equation}
  \label{eq:moments}
  \Mom{pqs}(Y) \EqDef \iiint_Y x^p y^q z^s dx dy dz\,.
\end{equation}
...
\begin{lemma}[\cite{Pottmann2007}, Théorème~2]
 ...
 \lambda_3$, nous avons l'expansion de Taylor suivante :
 \begin{align}
...
%
Nous souhaitons estimer la quantité $A_R(\vx) = \Area(B_R(\vx)\cap\Shape)$ de la
\RefDefinition{def:Volume} au pas de discrétisation $h$, où $B_R(\vx)$ est une
...
%
\begin{align}
  \AreaC(\DigF{B_R(\vx) \cap \Shape}{h}, h) & \EqDef h^2 \MCard( ( \frac{1}{h} \cdot ( B_R(\vx) \cap \Shape ) ) \cap \Z^2 ), \\
   & = h^2 \MCard( (\frac{R}{h} \cdot ( B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R} \cdot \Shape) ) \cap \Z^2 ), \\
   & = R^2 \frac{h^2}{R^2} \MCard( (\frac{R}{h} \cdot ( B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R} \cdot \Shape) ) \cap \Z^2 ), \\
   & = R^2 \AreaC( \DigF{B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R} \cdot \Shape}{h/R}, h/R ), \label{eq:Area_unitary_ball}
\end{align}
%
...
\begin{equation}
  ...
  \AreaC(\DigF{B_R(\vx) \cap \Shape}{h}, h) = R^2 \left(\Area( B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R} \cdot \Shape) + O( (h/R)^\beta) \right).
\end{equation}
%
Notons $SB(R)$ l'ensemble $B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R} \cdot
...
sous-ensemble $\BT B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R} \cdot \dS$) mais
...
B_1(\vx)$. Nous pouvons conclure qu'il existe quelques rayons $R_0$ tels que la
...
%
\begin{equation}
  A_R(\vx) = \Area( B_R(\vx) \cap \Shape ) = R^2 \Area( B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R} \cdot \Shape),
\end{equation}
%
nous obtenons :
%
\begin{equation}\label{eq:convergence-area-intersection}
  | \AreaC(\DigF{B_R(\vx) \cap \Shape}{h}, h) - A_R(\vx) | \le K_1 h^\beta R^{2-\beta}.
\end{equation}
%
...
définie dans le sous-ensemble $\BT B_1(\frac{1}{R} \cdot \vx) \cap \frac{1}{R}
...
\begin{equation}\label{eq:convergence-volume-intersection}
  | \VolC(\DigF{B_R(\vx) \cap \Shape}{h}, h) - V_R(\vx) | \le K_1 h^\gamma R^{3-\gamma}.
\end{equation}
%
...
\begin{definition}[Estimateur digital de courbure 2D par intégration.]\label{def:digital-2d-curvature}
  ...
  %
  \begin{equation}
    \forall 0 < h < R,\quad \CurvH{R}(\DigShape,\vx,h) \EqDef \frac{3\pi}{2R} - \frac{3\AreaC(B_{R/h}(\vx/h ) \cap \DigShape, h)}{R^3}\,.
  \end{equation}
  %
\end{definition}
%
\begin{definition}[Estimateur digital de courbure moyenne 3D par intégration.]
  ...
  %
  \begin{equation}
    \forall 0 < h < R,\quad \MeanCurvH{R}(\DigShape,\vx,h) \EqDef \frac{8}{3R} - \frac{4\VolC(B_{R/h}(\vx/h ) \cap \DigShape, h)}{\pi R^4}\,.
  \end{equation}
  %
\end{definition}
%
...
\begin{proof}
  ...
  $\DigF{B_R(\vx) \cap \Shape}{h}=B_{R/h}(\frac{1}{h} \cdot \vx) \cap
  \DSh$, nous obtenons pour $R < R_0$ :
  %
  \begin{equation}
    | \CurvH{R}(\DSh,\vx,h) - \Curv(\Shape,\vx) |
    = \left|\frac{3\pi}{2R}-\frac{3\AreaC(B_{R/h}(\vx / h) \cap \DSh, h)}{R^3} - \Curv(\Shape,\vx) \right|,
  \end{equation}
  %
  ...
  \begin{align}\label{eq-curvhat-error-bound-prelim}
    | \CurvH{R}(\DSh,\vx,h) - \Curv(\Shape,\vx) |
    &\le \left|\frac{3\pi}{2R}-\frac{3\Area(B_R(\vx) \cap \Shape)}{R^3} - \Curv(\Shape,\vx) \right| + 3K_1 \frac{h^\beta}{R^{1+\beta}} \nonumber \\
...
\begin{proof}
  ...
  $\DigF{B_R(\vx) \cap \Shape}{h}=B_{R/h}(\frac{1}{h} \cdot \vx) \cap \DSh$ nous
  permets d'obtenir pour $R < R_0$ :
  %
  \begin{equation}
    | \MeanCurvH{R}(\DSh,\vx,h) - \MeanCurv(\Shape,\vx) |
    = \left|\frac{8}{3R}-\frac{4\VolC(B_{R/h}(\vx / h) \cap \DSh, h)}{\pi R^4} - \MeanCurv(\Shape,\vx) \right|,
  \end{equation}
  %
  ...
  \begin{align}\label{eq-meancurvhat-error-bound-prelim}
    | \MeanCurvH{R}(\DSh,\vx,h) - \MeanCurv(\Shape,\vx) |
    &\le \left|\frac{8}{3R}-\frac{4\Vol(B_R(\vx) \cap \Shape)}{\pi R^4} - \MeanCurv(\Shape,\vx) \right| + \frac{4 K_1}{\pi} \frac{h^\gamma}{R^{1+\gamma}} \nonumber \\
  ...
  \begin{equation}\label{eq-meancurvhat-error-bound-prelim3}
    | \MeanCurvH{R}(\DSh,\vx,h) - \MeanCurv(\Shape,\vx) |
    \le K_2kh^\alpha + \frac{4K_1}{\pi k^{1+\gamma}}h^{\gamma - \alpha(1+\gamma)}.
  \end{equation}
  %
...
%
Soit $\Shape$ un domaine compact de $\R^2$ tel que son bord $\dS$ est $C^2$ à
...
%
\begin{equation}
  | \AreaC(\DigF{B_R(\vxH) \cap \Shape}{h}, h) - A_R(\vxH) | \le K_1 h^\beta R^{2-\beta},
\end{equation}
%
ce qui implique, avec l'\RefEquation{eq-area-shift-error} :
%
\begin{equation}\label{eq-shift-B-R-error-bound-2D}
  | \AreaC(\DigF{B_R(\vxH) \cap \Shape}{h}, h) - A_R(\vx) |  \le K_1 h^\beta R^{2-\beta} +  2 R \vt (1 + O(R^2) + O(\vt)).
\end{equation}
%
...
%
\begin{equation}
  | \VolC(\DigF{B_R(\vxH) \cap \Shape}{h}, h) - V_R(\vxH) | \le K_1 h^\gamma R^{3-\gamma},
\end{equation}
%
ce qui implique, avec l'\RefEquation{eq-vol-shift-error} :
%
\begin{equation}\label{eq-shift-B-R-error-bound-3D}
  | \VolC(\DigF{B_R(\vxH) \cap \Shape}{h}, h) - V_R(\vx) |  \le K_1 h^\gamma R^{3-\gamma} +  R^2 \pi \vt (1 + O(R^2) + O(\vt)).
\end{equation}
%
...
le meilleur $\alpha$ possible. Les ordres de grandeur étant les mêmes qu'en 2D,
nous obtenons $\alpha = \frac{\gamma}{2 + \gamma}$ si $\alpha' \ge \frac{3
\gamma}{1 + \gamma}$, sinon $\alpha = \frac{\alpha'}{3}$
%
...
\begin{definition}{\textbf{(Estimateurs de courbure discrète 2D $\CurvH{R}(\DigShape,\vp)$ et de courbure discrète moyenne 3D $\MeanCurvH{R}(\DigShape,\vp)$).}}
  \begin{equation}
    \CurvH{R}(\DigShape,\vp) \EqDef \frac{1}{h} \left( \frac{3 \pi}{2 R_d}
     - \frac{3 A_{R_d}(\vp)}{{R_d}^3} \right),
     \quad \MeanCurvH{R}(\DigShape,\vp) \EqDef \frac{1}{h} \left( \frac{8}{3 R_d}
      - \frac{4V_{R_d}(\vp)}{{\pi R_d}^4} \right).
  \end{equation}
  \label{def:ii-2d-3d-mean}
\end{definition}

\begin{theorem}{\textbf{(Convergence des estimateurs de courbure discrète 2D $\CurvH{R}(Z,p)$ et de courbure discrète moyenne 3D $\MeanCurvH{R}(Z,p)$).}}
  \begin{equation}
    \CurvH{R}(\DSh,p) = \Curv(\Shape,\vx) + O(h^{\frac{1}{3}}),
    \quad \MeanCurvH{R}(\DSh,p) = \MeanCurv(\Shape,\vx) + O(h^{\frac{1}{3}}).
  \end{equation}
  \label{theo:ii-2d-3d-mean-conv}
\end{theorem}
...
\begin{align}
  \hat{m}_{p,q,s}(&\DigF{A}{h},h) = h^{3+{\sigma}}M_{p,q,s}\left
  ( (\frac{1}{h} \cdot \Ball{R}{\vx} \cap \Shape) \cap \Z^3 \right) \nonumber \\
  & = h^{3+{\sigma}} M_{p,q,s}\left( \frac{R}{h}\cdot(
  \Ball{1}{\frac{1}{R} \cdot \vx} \cap \frac{1}{R} \cdot \Shape) \cap \Z^3 \right) \nonumber \\
  &= R^{3+{\sigma}} \left( \frac{h}{R} \right)^{3+{\sigma}} \hspace{-0.3cm} M_{p,q,s} \left( \DigF{ B_1(\frac{1}{R}\cdot \vx)\cap\frac{1}{R}\cdot \Shape}{h/R} \right) \nonumber\\
&= R^{3+{\sigma}}\hat{m}_{p,q,s}\left( \DigF{ B_1(\frac{1}{R}\cdot \vx)\cap\frac{1}{R}\cdot \Shape}{h/R}, {\frac{h}{R}} \right).\label{eq:fin}
\end{align}

\begin{align}
\hat{m}_{p,q,s}(\DigF{A}{h},h) &= R^{3+{\sigma}}
m_{p,q,s}\left( \Ball{1}{\frac{1}{R}\cdot \vx} \cap \frac{1}{R}\cdot \Shape
\right) \nonumber\\&\quad\quad\quad\quad\quad+  R^{3+{\sigma}}
O \left( \frac{h}{R} \right)^{\mu_{{\sigma}}}  \nonumber \\
 &= m_{p,q,s}( \Ball{R}{\vx} \cap \Shape ) + O( R^{3+{\sigma}-\mu_{{\sigma}}} h^{\mu_{{\sigma}}}) \nonumber \\
 &= m_{p,q,s}( A ) + O( R^{3+{\sigma}-\mu_{{\sigma}}} h^{\mu_{{\sigma}}}).\label{eq:conv-digital-moment}
\end{align}


\begin{align}
  \hat{J}( \DigShape,h )\EqDef & \left [
    \begin{array}{ccc}
      \hat{m}_{2,0,0}(\DigShape,h) &       \hat{m}_{1,1,0}(\DigShape,h) &      \hat{m}_{1,0,1}(\DigShape,h)\\
      \hat{m}_{1,1,0}(\DigShape,h) &   \hat{m}_{0,2,0}(\DigShape,h) &      \hat{m}_{0,1,1}(\DigShape,h)\\
      \hat{m}_{1,0,1}(\DigShape,h) & \hat{m}_{0,1,1}(\DigShape,h)  & \hat{m}_{0,0,2}(\DigShape,h)
    \end{array}
    \right ] \nonumber\\
  & -\frac{1}{\hat{m}_{0,0,0}(\DigShape,h)}
  \left [ \begin{array}{c}
      \hat{m}_{1,0,0}(\DigShape,h) \\ \hat{m}_{0,1,0}(\DigShape,h) \\ \hat{m}_{0,0,1}(\DigShape,h)\\
    \end{array} \right ]
  \otimes
  \left [ \begin{array}{c}
      \hat{m}_{1,0,0}(\DigShape,h) \\ \hat{m}_{0,1,0}(\DigShape,h) \\ \hat{m}_{0,0,1}(\DigShape,h)\\
    \end{array} \right ]^T.
\label{def:digital-covariance-matrix}
\end{align}

\begin{theorem}{\textbf{(Multigrid convergence of digital covariance matrix).}}
  \label{thm:conv-cov-matrix}
  Let $X \in \Shapes$. Then, there exists some constant $h_X$, such
  that for any grid step $0< h < h_X$, for arbitrary $x \in \R^3$, for
  arbitrary $R \ge h$, with non-empty $A(R,x) \EqDef \Ball{R}{x} \cap
  X$, we have:
  \begin{align*}
    \| \hat{J}( \DigF{A(R,x)}{h}, h ) - J( A(R,x) ) \|
    &\le \sum_{i=0}^2 O(R^{5-\mu_i}h^{\mu_i}).
  \end{align*}
  The constants hidden in the big O do not depend on the shape size or
  geometry. {$\|\cdot\|$ denotes the spectral norm on matrices. }
\end{theorem}

\begin{lemma}
\label{lem:moment-pos-error}
  For any measurable subset $\Shape \subset \R^3$ and any vector $\mathbf{t}$ with
  norm $t \EqDef \| \mathbf{t} \|_2 \le R$, we have for $0 \le p+q+s
  \EqDef {\sigma} \le 2$:
  \begin{align}
    m_{p,q,s}(\Ball{R}{x+\mathbf{t}} \cap X ) & = m_{p,q,s}(\Ball{R}{x} \cap X ) \nonumber\\
    &+ \sum_{i=0}^{{\sigma}} O(\|x\|^i tR^{2+{\sigma}-i}).
    %% m_{1,0,0}(\Ball{R}{x+\mathbf{t}} \cap X ) &= m_{1,0,0}(\Ball{R}{x} \cap X ) + O(R^3t)\\
    %% m_{2,0,0}(\Ball{R}{x+\mathbf{t}} \cap X ) &= m_{2,0,0}(\Ball{R}{x} \cap X ) + O(R^4t)\\
    %% m_{1,1,0}(\Ball{R}{x+\mathbf{t}} \cap X ) &= m_{2,0,0}(\Ball{R}{x} \cap X ) + O(R^4t).
  \end{align}
\end{lemma}
